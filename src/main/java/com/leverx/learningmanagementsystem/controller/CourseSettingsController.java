...
public class CourseSettingsController {
    ...
    public CourseSettingsDto getCourseSettings(@PathVariable UUID id) {
        return courseSettingsService.getCourseSettings(id);
    }

    @PutMapping("/{id}")
    @ResponseStatus(HttpStatus.OK)
    public CourseSettingsDto updateCourseSettings(@PathVariable UUID id,
                                  @RequestBody UpdateCourseSettingsRequest request) {
        return courseSettingsService.updateCourseSettings(id, request);
    }
